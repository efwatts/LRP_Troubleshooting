...
def parse_args():
    parser = argparse.ArgumentParser(description="Color BED12 based on PSI values")
    ...
    parser.add_argument('--psi_column_NBM', default='avg_PSI_NBM')
    parser.add_argument('--psi_column_mutant', default='avg_PSI_SRSF2WT')
    parser.add_argument('--pval_column', default='SRSF2WT-NBM_p.value')
...
def get_coords_from_name(name_field):
    return name_field.split(':alternative')[0].split(':', 1)[1]
...
def main():
    ...
    psi_df[args.psi_column_NBM] = pd.to_numeric(psi_df[args.psi_column_NBM], errors='coerce')
    ...
    psi_df['abs_NBM'] = psi_df[args.psi_column_NBM].abs().clip(0, 1)
    psi_df['rgb_NBM'] = psi_df['abs_NBM'].apply(psi_to_rgb)
    coord_to_NBM = psi_df.set_index('Coordinates')[[args.psi_column_NBM, args.pval_column]].copy()
    coord_to_NBM['psi'] = coord_to_NBM[args.psi_column_NBM]
    coord_to_NBM['pval'] = psi_df.set_index('Coordinates')[args.pval_column]
    coord_to_NBM['rgb'] = psi_df.set_index('Coordinates')['rgb_NBM']
    ...
    generate_bed(args.output_prefix + '_NBM.bed12', coord_to_NBM.to_dict(orient='index'), args.input_bed, f"{args.event_type}_NBM")
    generate_bed(args.output_prefix + '_SRSF2WT.bed12', coord_to_mut.to_dict(orient='index'), args.input_bed, f"{args.event_type}_SRSF2WT")
