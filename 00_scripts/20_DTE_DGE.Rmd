...
base_project_dir <- "/Volumes/sheynkman/projects/LRP_PIFF_dev"

# Define sub paths
counts_file <- file.path(base_project_dir, "01_isoseq", "HAEC", "collapse", "merged.collapsed.flnc_count.txt")
sqanti_table_file <- file.path(base_project_dir, "03_filter_sqanti", "HAEC", "HAEC_classification.5degfilter.tsv")
...
sqanti_table <- read.delim(sqanti_table_file, header = TRUE, sep = "\t")
...
Note: I'm allowing for a high FDR value, in part because this is a test, and I don't know anything about the input conditions
I am allowing the default normalization here, but we should probably test with different normalization methods
...
tab_fdr  <- top_500_tab %>% filter(FDR < 0.9)
...
Note again: I am allowing for a high FDR in this test. In a real analysis <0.05 or <0.1 is more appropriate
```{r}
...
# Filter DE transcripts based on FDR < 0.9
deg_transcripts_filtered <- deg_transcripts %>% filter(FDR < 0.9)

# Save filtered DE transcripts
write.table(deg_transcripts_filtered, 
            file = file.path(output_table_subfolder, "transcripts_DEG_results_filtered_FDR09.txt"), 
            sep = "\t", quote = FALSE, row.names = TRUE)

# Print number of filtered transcripts
cat("Number of DE transcripts with FDR < 0.9:", nrow(deg_transcripts_filtered), "\n")
```
...
top_500_fdr  <- top_500_tab %>% filter(FDR < 0.9)
...
cpm_long <- cpm_df %>%
  ...
  mutate(group = case_when(
    str_detect(sample, "BioSample_[1-3]") ~ "condition1",
    str_detect(sample, "BioSample_[4-6]") ~ "condition2",
    TRUE ~ NA_character_
  ))
...
```
Plot isoform expression for a gene of interest (condition1 vs condition2)
```{r}
...
gene_of_interest <- "ENSG00000188976.11"  # Replace with your gene ID
...
plot_title <- if (length(symbol) > 0 && !is.na(symbol) && symbol != "") {
  paste("Isoform Expression for", symbol, "(condition2 vs condition1)")
} else {
  paste("Isoform Expression for", gene_of_interest, "(condition2 vs condition1)")
}
...
# Use default group ordering (condition1 on the left)
...
dynamic_transcripts$group <- ifelse(dynamic_transcripts$logFC > 0, "condition1", "condition2")
dynamic_transcripts$group <- factor(dynamic_transcripts$group, levels = c("condition2", "condition1"))
...
for (gene in genes_to_plot) {
  ...
  title_label <- if (!is.na(gene_symbol) && gene_symbol != "") {
    paste("Expression of", gene_symbol, "Isoforms (condition2 vs condition1)")
  } else {
    paste("Expression of", gene, "Isoforms (condition2 vs condition1)")
  }
  ...
  filename <- paste0(safe_name, "_isoform_expression_condition2_vs_condition1.pdf")
...
annotation_colors <- list(
  Group = c("condition1" = "#336B87", "condition2" = "#E99787")
)

# Define group annotations based on sample names
sample_names <- unique(dynamic_cpm_long$sample)
annotation_col <- data.frame(
  Group = ifelse(str_detect(sample_names, "BioSample_[1-3]"), "condition1", "condition2")
)
...
p_transcripts_top500 <- pheatmap(transcript_matrix_log2_top500,
                                 ...
                                 main = "Top 500 Dynamic Transcripts (condition2 vs condition1)")

pdf(file.path(plots_subfolder, "dynamic_transcript_heatmap_top500_condition2_vs_condition1.pdf"), width = 8, height = 10)
...
p_genes_top500 <- pheatmap(gene_matrix_log2_top500,
                           ...
                           annotation_col = annotation_col,
                           annotation_colors = annotation_colors,
                           main = "Top 500 Dynamic Genes (condition2 vs condition1)")

pdf(file.path(plots_subfolder, "dynamic_gene_heatmap_top500_condition2_vs_condition1.pdf"), width = 8, height = 10)
...
p_transcripts_top50 <- pheatmap(transcript_matrix_log2_top50,
                                ...
                                annotation_col = annotation_col,
                                annotation_colors = annotation_colors,
                                main = "Top 50 Dynamic Transcripts (condition2 vs condition1)")

pdf(file.path(plots_subfolder, "dynamic_transcript_heatmap_top50_condition2_vs_condition1.pdf"), width = 8, height = 10)
...
p_genes_top50 <- pheatmap(gene_matrix_log2_top50,
                          ...
                          annotation_col = annotation_col,
                          annotation_colors = annotation_colors,
                          main = "Top 50 Dynamic Genes (condition2 vs condition1)")

pdf(file.path(plots_subfolder, "dynamic_gene_heatmap_top50_condition2_vs_condition1.pdf"), width = 8, height = 10)
print(p_genes_top50)
dev.off()
```
...
# rename clearly to work around errors I've been encountering
...
colnames(average_frac_abundance)
...
# ------------------------------------------
# Plot Top 50 Isoforms
# ------------------------------------------
top50_isoforms <- average_frac_abundance %>%
  pivot_wider(
    names_from = group,
    values_from = avg_abundance,
    values_fill = list(avg_abundance = 0)
  ) %>%
  mutate(diff = abs(condition1 - condition2)) %>%
  arrange(desc(diff)) %>%
  slice(1:50)

top50_long <- top50_isoforms %>%
  pivot_longer(cols = c(condition1, condition2), names_to = "group", values_to = "avg_abundance") %>%
...
p_top50_isoforms <- ggplot(top50_long, aes(x = reorder(isoform_label, -avg_abundance), y = avg_abundance, fill = group)) +
  ...
  labs(title = "Top 50 Most Differentially Expressed Isoforms (condition2 vs condition1)",
       x = "Isoform (Gene:Transcript)", y = "Average Fractional Abundance") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_y_continuous(expand = c(0, 0))

ggsave(file.path(plots_subfolder, "Top50_Differential_Isoforms_condition2_vs_condition1.pdf"),
...
custom_group_colors <- c("condition1" = "#336B87", "condition2" = "#E99787")
...
for (gene in genes_to_plot_filtered) {
  ...
  plot_title <- if (!is.na(gene_symbol) && gene_symbol != "") {
    paste("Stacked Isoform Usage for", gene_symbol, "(condition2 vs condition1)")
  } else {
    paste("Stacked Isoform Usage for", gene, "(condition2 vs condition1)")
  }
  ...
  # Save
  ggsave(file.path(plots_subfolder, paste0(safe_name, "_stacked_barplot_condition2_vs_condition1.pdf")),
         p, width = 6, height = 4)
}
p
```
...
custom_colors <- c("condition1" = "#336B87", "condition2" = "#E99787")
...
for (gene in genes_to_plot) {
  ...
  # Save each plot
  ggsave(file.path(plots_subfolder, paste0(gene, "_stacked_barplot_condition_vs_condition2.pdf")),
         plot = p, width = 6, height = 5)
}
```
...
for (gene in genes_to_plot) {
  ...
  p_rep <- ggplot(gene_data_rep, aes(x = Sample_Group, y = fractional_abundance, fill = transcript_id)) +
    geom_bar(stat = "identity", position = "stack") +
    labs(title = paste("Replicate-level Stacked Barplot for", gene, "(condition1 vs condition2)"),
         x = "Sample", 
