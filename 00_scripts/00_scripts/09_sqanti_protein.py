...
def read_in_custom_orf_calls_into_orfDict(orf_file):
    ...
    # stick with same format of orfDict as expected in SQANTI
    orfDict = {}  # pb_acc -> myQueryProtein
    for line in open(orf_file):
        if line.startswith('ID') or line.startswith('isoform'):
            continue  # skip header line
        wds = line.strip().split('\t')
        
        pb_acc = wds[10]            # NEW: pb_acc moved to column 10
        cds_start = int(wds[4])     # NEW: cds_start is now column 4
        cds_end = int(wds[5])       # NEW: cds_end is now column 5
        orf_length = int(wds[6])    # NEW: orf_len is now column 6
        seq = ''                    # sequence is still blank for now
        transcript_len = int(wds[1])  # transcript length still column 1
