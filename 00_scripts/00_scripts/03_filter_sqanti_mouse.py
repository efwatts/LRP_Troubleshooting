...
def save_filtered_sqanti_gtf(gtf_file, filtered_isoforms, output_path):
    ...
    with open(gtf_file, "r") as ifile, open(output_path, "w") as ofile:
        for line in ifile:
            ...
            if match and match.group(1) in filtered_isoforms:
                ofile.write(line)

def save_filtered_sqanti_fasta(fasta_file, filtered_isoforms, output_path):
    logging.info("Saving FASTA")
    filtered_sequences = [record for record in SeqIO.parse(fasta_file, "fasta")
...
def filter_protein_coding(classification, protein_coding_filename, ensmusg_gene_filename):
    ...
    return classification[classification['associated_gene'].isin(protein_coding_gene_ids)]
...
def main():
    ...
    if is_protein_coding_filtered:
        if not args.protein_coding_genes or not args.ensmusg_gene:
            raise ValueError("Both --protein_coding_genes and --ensmusg_gene must be provided when filtering for protein coding.")
