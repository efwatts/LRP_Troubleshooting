...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare rMATS skipped exon to SUPPA SE events with coordinate tolerance")
    ...
    parser.add_argument("--tolerance", type=int, default=10, help="Max difference in exon start/end to count as a match (default: 10bp)")
    return parser.parse_args()

def parse_event_coords(event_str):
    """
    Extract coordinates from SUPPA SE event_id string.
    Format: chr6:47541844-47541970:47542365-47542406:-
    """
...
def main():
    ...
    # Filter for SE events and the target gene
    ...
    for _, row in df.iterrows():
        ...
        if coords and coords["chr"] == args.chrom and coords["strand"] == args.strand:
            ...
            if delta_start <= args.tolerance and delta_end <= args.tolerance:
                matched.append({
    ...
    if matched:
        ...
        print("No matching events found within tolerance window.")
