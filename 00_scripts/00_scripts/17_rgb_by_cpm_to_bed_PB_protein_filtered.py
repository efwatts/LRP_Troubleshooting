...
def extract_gene_pb_cpm(acc):
    try:
        ...
        category = parts[2]
        cpm = float(parts[3])
        return gene, pb_id, category, cpm
    except (IndexError, ValueError):
        return None, None, None, float("nan")

def make_acc_label_with_pb(gene, pb_id, category, cpm_str, frac_str):
    return f"{gene}|{pb_id}|{category}|{cpm_str}|{frac_str}"
...
def add_rgb_colors(bed_file, light_rgb, dark_rgb, output_file):
    ...
    parsed.columns = ['gene', 'tx_id', 'category', 'cpm']
    ...
    bed['acc_full'] = bed.apply(
        lambda row: make_acc_label_with_pb(row['gene'], row['tx_id'], row['category'], row['cpm_str'], row['frac_str']),
        axis=1
    )
...
def main():
    ...
    add_rgb_colors(args.input_bed, light_rgb, dark_rgb, args.output_file)
