    ...
    with open(input_gtf_path, "r") as in_gtf, open(output_gtf_path, "w") as out_gtf:
        for line in in_gtf:
            ...
            if match:
                ...
                if transcript_id in dropout_ids:
                    out_gtf.write(line)
...
def filter_with_reason(df, filter_fn, reason, dropout_reasons):
    pre_ids = set(df['isoform'])
    df_filtered = filter_fn(df)
    post_ids = set(df_filtered['isoform'])
    dropped = pre_ids - post_ids
    for iso in dropped:
        dropout_reasons[iso] = reason
    return df_filtered
...
def main():
    ...
    parser.add_argument("--ensg_gene", required=False)
    ...
    dropout_dir = os.path.join(args.output_dir, "dropout")
    ...
    sqanti_df = sqanti_df[sqanti_df['associated_gene'].str.startswith("ENSG")]
    ...
    if is_protein_coding_filtered:
        sqanti_df = filter_with_reason(sqanti_df, 
                                       lambda df: filter_protein_coding(df, args.protein_coding_genes, args.ensg_gene),
                                        "not_protein_coding", 
                                        dropout_reasons)

    if is_intra_polyA_filtered:
        sqanti_df = filter_with_reason(sqanti_df, 
                                        lambda df: filter_intra_polyA(df, args.percent_A_downstream_threshold),
                                        "poor_polyA", 
                                        dropout_reasons)
    ...
    if args.structural_categories_level in structural_categories:
        pre = set(sqanti_df['isoform'])
        sqanti_df = sqanti_df[sqanti_df['structural_category'].isin(structural_categories[args.structural_categories_level])]
        post = set(sqanti_df['isoform'])
        for iso in pre - post:
            dropout_reasons[iso] = "structural_category_filtered"

    kept_ids = set(sqanti_df['isoform'])
    dropout_ids = original_ids - kept_ids

    dropout_reason_df = pd.DataFrame({
        "isoform": list(dropout_ids),
        "dropout_reason": [dropout_reasons.get(i, "unknown") for i in dropout_ids]
        })
    dropout_reason_df.to_csv(os.path.join(dropout_dir, "dropout_reasons.tsv"), sep="\t", index=False)
    ...
    write_dropout_classification(dropout_ids, args.classification_file,
                                  os.path.join(dropout_dir, f"dropout_{base_class_name}"))
    write_dropout_fasta(dropout_ids, args.corrected_fasta,
                        os.path.join(dropout_dir, f"dropout_{base_fasta_name}"))
    write_dropout_gtf(dropout_ids, args.corrected_gtf,
                      os.path.join(dropout_dir, f"dropout_{base_gtf_name}"))
