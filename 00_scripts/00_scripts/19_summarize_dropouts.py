...
def summarize_dropouts(dropout_df, output_prefix):
    ...
    # Map prettier labels
    dropout_df["Condensed_Reason"] = dropout_df["Specific_Reason"].apply(lambda x: map_Condensed_label(x))

    dropout_df.to_csv(f"{output_prefix}_dropout_summary_detailed.tsv", sep="\t", index=False)

    # Global barplot
    reason_counts = dropout_df["Condensed_Reason"].value_counts()
    ...
    for step in dropout_df["Step"].unique():
        ...
        step_reason_counts = step_df["Condensed_Reason"].value_counts()
