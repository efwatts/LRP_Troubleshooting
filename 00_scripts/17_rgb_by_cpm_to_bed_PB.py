...
def make_acc_label_with_pb(gene, pb_id, cpm_str, frac_str, pb_to_ens):
...
def add_rgb_colors(bed_file, light_rgb, dark_rgb, output_file, pb_to_ens):
    ...
    bed[['gene', 'tx_id', 'cpm']] = bed['acc_full'].apply(lambda x: pd.Series(extract_gene_pb_cpm(x)))
    ...
    # Score is optional but useful
    bed['score'] = ((bed['frac'] - min_frac) / (max_frac - min_frac) * 1000).fillna(0).astype(int)

    # Replace acc_full with updated PBID-based label
    bed['acc_full'] = bed.apply(
        lambda row: make_acc_label_with_pb(row['gene'], row['tx_id'], row['cpm_str'], row['frac_str'], pb_to_ens),
        axis=1
    )
    ...
    # Fix filter_names: use 'itemRGB', not 'rgb'
