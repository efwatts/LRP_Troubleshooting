...
def make_acc_label(gene, tx_id, cpm, frac, is_novel):
    # Truncate CPM and Frac to 2 decimal places
    label = f"{gene}|{tx_id}|{cpm:.2f}|{frac:.2f}"
...
def add_rgb_colors(input_bed, output_file):
    ...
    # Generate new acc_full as PBID|CPM|Frac[*] with 2 decimal places
    bed['acc_full'] = bed.apply(lambda row: make_acc_label(row['gene'], row['tx_id'], row['cpm'], row['frac'], row['is_novel']), axis=1)

    # Optional: parse class from transcript ID if embedded
