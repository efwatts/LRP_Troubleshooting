...
def sqtab(sqanti_out, ensg_to_gene, enst_to_isoname):
    ...
    fdata.loc[:, 'cpm'] = 1000000 * fdata['fl_cts'] / fdata['fl_cts'].sum(skipna=True)
    ...
    df = fdata[['gene']]
    fdata.loc[:, 'gene'] = fdata['gene'].map(gdict).fillna(df['gene'])

    # Drop cases like ENSG00000242861.1_ENSG00000196187.12 
    fdata = fdata[~fdata['gene'].str.contains('_')]
    ...
    fdata.loc[:, 'transcript'] = fdata['transcript'].map(tdict).fillna(df2['transcript'])
